  ...
  template: `
      ...
      <div class="rd-header__inner">
        ...
        @if (raffle()) {
          <a [routerLink]="['/mis-numeros']" [queryParams]="{ slug: raffle()!.slug }"
            ...
            <i class="bi bi-ticket-perforated me-1"></i>Consultar mis números
          </a>
        }
      </div>
    </header>
                ...
                @if (selected().length === 0) {
                  ...
                  @if (reservationSuccess()) {
                    <div class="alert alert-success text-center">
                      ...
                      <p class="mb-0 small mt-1">Tu reserva expira en 30 minutos. Contactá al organizador para confirmar el pago.</p>
                    </div>
                  } @else {
                    @if (reservationError()) {
    ...
    @if (!loading() && !raffle()) {
      <div class="container py-5 text-center">
        ...
        <p class="text-muted">El enlace puede ser incorrecto o la rifa aún no está disponible.</p>
      </div>
    }
  `,
  styles: [`
...
export class RaffleDetail implements OnInit, OnDestroy {
  ...
  protected readonly allImages = computed(() => {
    const r = this.raffle();
    if (!r) return [];
    const imgs = [...r.images].sort((a, b) => a.displayOrder - b.displayOrder);
    if (r.prize?.imageUrl && !imgs.some(i => i.url === r.prize!.imageUrl)) {
      imgs.unshift({ url: r.prize.imageUrl, altText: r.prize.name, coverImage: false, displayOrder: -1 });
    }
    return imgs;
  });
  ...
  private startSlideTimer(): void {
    ...
    this.slideTimer = setInterval(() => {
      this.activeImg.update(i => (i + 1) % this.allImages().length);
    }, 4000);
  }

  protected prevImg(): void {
    clearInterval(this.slideTimer);
    this.activeImg.update(i => (i - 1 + this.allImages().length) % this.allImages().length);
    this.startSlideTimer();
  }
  ...
  protected isAvailable(n: NumberInfo): boolean { return n.status === 'AVAILABLE'; }
  protected isSelected(num: number): boolean     { return this.selected().includes(num); }
